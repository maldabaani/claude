...
class _TemplatesScreenState extends ConsumerState<TemplatesScreen> {
  ...
  void _showForm([Map<String, dynamic>? item]) {
    showDialog(context: context, builder: (_) => _TemplateDialog(item: item, onSave: (data) async {
      if (item != null) await _api.put('${ApiEndpoints.templates}/${item['id']}', data: data);
      else await _api.post(ApiEndpoints.templates, data: data);
      await _load();
    }));
  }
  ...
  Future<void> _delete(String id) async {
    final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(
      title: const Text('Delete Template'), content: const Text('Are you sure?'),
    ...
    if (ok == true) { await _api.delete('${ApiEndpoints.templates}/$id'); await _load(); }
  }

  @override
  Widget build(BuildContext context) => Scaffold(
    backgroundColor: AppColors.background,
    appBar: AppBar(backgroundColor: AppColors.surface, elevation: 0, title: Text('Templates (${_items.length})')),
        ...
        : RefreshIndicator(onRefresh: _load, child: ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              return Container(padding: const EdgeInsets.all(14),
                ...
                child: Row(children: [
                  const CircleAvatar(radius: 20, backgroundColor: AppColors.primaryLight, child: Icon(Icons.description_outlined, color: AppColors.primary, size: 20)),
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(d['name'] ?? '', style: const TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                    Text(d['subject'] ?? '', style: const TextStyle(fontSize: 12, color: AppColors.textSecondary), overflow: TextOverflow.ellipsis),
                  ])),
...
class _TemplateDialog extends StatefulWidget {
  ...
  const _TemplateDialog({this.item, required this.onSave});
  @override
...
class _TemplateDialogState extends State<_TemplateDialog> {
  ...
  void initState() {
    ...
    if (widget.item != null) {
      ...
      _subjectCtrl.text = widget.item!['subject'] ?? '';
      _bodyCtrl.text = widget.item!['body'] ?? '';
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      await widget.onSave({'name': _nameCtrl.text.trim(), 'subject': _subjectCtrl.text.trim(), 'body': _bodyCtrl.text.trim()});
  ...
  Widget build(BuildContext context) => AlertDialog(
    title: Text(widget.item != null ? 'Edit Template' : 'New Template'),
    content: SingleChildScrollView(child: Form(key: _formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [
      TextFormField(controller: _nameCtrl, decoration: const InputDecoration(labelText: 'Template Name'), validator: (v) => (v?.isEmpty ?? true) ? 'Required' : null),
      const SizedBox(height: 12),
      TextFormField(controller: _subjectCtrl, decoration: const InputDecoration(labelText: 'Subject'), validator: (v) => (v?.isEmpty ?? true) ? 'Required' : null),
      const SizedBox(height: 12),
      TextFormField(controller: _bodyCtrl, decoration: const InputDecoration(labelText: 'Body'), maxLines: 6, validator: (v) => (v?.isEmpty ?? true) ? 'Required' : null),
    ]))),
    actions: [
